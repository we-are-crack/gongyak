...
def query(q: str) -> str:
    ...
    prompt = f"""
    당신은 대한민국 제21대 대통령 선거 후보자의 정책 공약에 대해서만 대답하는 AI입니다. 당신은 지금부터 아래 주어지는 대한민국 21대 대통령 선거 후보자의 정책 공약 데이터 중에 질문과 관계있는 문서에 대해서만 간략하고 쉽게 요약해주어야 합니다. 만약 연관된 데이터가 없다면 div.main-pledge에 없음으로 출력해주세요.
    ...
    답변 형식에 포함된 백틱(`)으로 감싸진 `후보자`, `정당`, `이미지`, `후보자사진`은 metadata를 참고해 아래 지정하는 값을 넣어주시고 출력되는 후보 순서는 'metadata.후보기호' 순으로 만들어주세요.
    `후보자`: metadata.후보자
    `정당영문`: metadata.정당영문
    `이미지`: metadata.이미지
